...
public class ClefNotesOptionDialog extends JDialog implements ActionListener
{
	...
	public ClefNotesOptionDialog(Font f, ResourceBundle b, Preferences p)
	{
		...
		clefsPanel.add(tenorClef);		
		  ...
		  */
		JPanel notesPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 16, 6));
		notesPanel.setBackground(Color.white);
		notesPanel.setBounds((clefSelWidth * 2) + 15, 0, getWidth() - (clefSelWidth * 2) - 20, getHeight() - 75);
		notesPanel.setPreferredSize(new Dimension(getWidth() - (clefSelWidth * 2) - 20, getHeight() - 90));

		// ****** Sub panel of notesPanel. Contains accidental selection (label + combobox)
		RoundPanel accidentalsPanel = new RoundPanel();
		accidentalsPanel.setLayout(new FlowLayout(FlowLayout.CENTER, 0, 6));
		...
		notesTypePanel.setLayout(new FlowLayout(FlowLayout.CENTER, 14, 6));
...
// 0xAB Begin
                hintCB = new JCheckBox("Show Text Hint");
                noteOn = Integer.parseInt(appPrefs.getProperty("showHint")); 
		if (noteOn == 1 || noteOn == -1)
			hintCB.setSelected(true);
//hintCB.setFont(appFont.deriveFont(50f));
                
                notesTypePanel.add(hintCB);
	...
	public void actionPerformed(ActionEvent ae)
	{
		if (ae.getSource() == okButton)
		{
			...
			if (silenceCB.isSelected() == true) appPrefs.setProperty("silenceNote", "1");
                        ...
                        //0xAB
                        if (hintCB.isSelected() == true) appPrefs.setProperty("showHint", "1");
			else appPrefs.setProperty("showHint", "0");
