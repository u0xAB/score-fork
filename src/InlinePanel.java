...
public class InlinePanel extends JPanel implements ActionListener 
{
	...
	private class InlineGameThread extends Thread 
	{
        ...
        private void readScriptNote() {
            Note newNote = ScriptVector.INSTANCE.notes.get(ScriptVector.INSTANCE.index());
            newNote.duration = 0; // set duration to 0 not to mess up X position
            newNote.xpos = noteXStartPos;
            setLevel(newNote);
            if (gameType != appPrefs.INLINE_SINGLE_NOTES)
        ...
        void setLevel(Note note)
        {
            int altShift = note.altType;
            note.altType = 0;
            note.pitch -= altShift;
            if (note.secondRow)
                note.level = 24 - inlineNG.getIndexFromPitch(inlineNG.BASS_CLEF_BASEPITCH, note.pitch, true);
            else 
                note.level = 24 - inlineNG.getIndexFromPitch(inlineNG.TREBLE_CLEF_BASEPITCH, note.pitch, false);
            note.altType = altShift;
            note.pitch += altShift;
        }
}
